...
public class myAdapter extends RecyclerView.Adapter<myAdapter.myViewHolder>{
    private ArrayList<MyObject> Object_List;
    private Context context;

    public myAdapter(ArrayList<MyObject> Object_List, Context context){
    ...
    public int getItemViewType(int position) {
        ...
        String username = Object_List.get(position).getMyUser();
        String description = Object_List.get(position).getMyDescription();
    ...
    public myViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        View view;
        if (viewType == 0) {
            view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.activity_recycler, viewGroup, false);
            return new myViewHolder(view);
        }
        if (viewType == 1) {
            view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.activity_bigrecycler, viewGroup, false);
            return new myViewHolder(view);
        }
        return null;
    }
